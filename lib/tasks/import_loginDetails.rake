namespace :import_loginDetail do
  ...
  task :import_login => :environment do
    ...
    years = Dir.entries('public/Network Sheets')
    years.each do |month_file|
      unless month_file == "." || month_file == ".."
        months = Dir.entries('public/Network Sheets/'+month_file)
        months.each do |filesheets|

          unless filesheets == "." || filesheets == ".."
            files  = Dir.entries('public/Network Sheets/'+ month_file + "/" + filesheets)
            files.each do |file|
              begin
              productiveHours = nil
              unless (file == '.' || file == '..')
              ...
              unless (file == '.' || file == '..') || (loginFiles.present?)
                xlsx = Roo::Spreadsheet.open('public/Network Sheets/'+ month_file + "/" + filesheets + "/" +file)
                data = xlsx.sheet(0)
                timeSheetDate = nil
                employeeIdArray = []
                previous_Time = nil
                  ...
                  unless index == 0
                    ...
                    begin
                      ...
                      if dateAndTime.count == 2
                        e.login_time = (timeSplit.to_time).to_s(:time)
                      else
